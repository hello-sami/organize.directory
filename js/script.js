...
const breakingNews = {
     ...
     displayHeadlines() {
          ...
          headlinesContent.innerHTML = this.headlines
               .map(
                    (item, index) => `
...
function showCityPage(cityName, stateName) {
     const cityInitiatives = initiatives.filter(
          (initiative) =>
...
function showAllStates() {
     // Check if we're on the location page with a pre-populated list
     const existingStatesList = document.getElementById(
          "states-list-container"
     );
     if (
          existingStatesList &&
          existingStatesList.getAttribute("data-populated") === "true"
     ) {
          // The states list is already populated in HTML, no need to generate another one
          return;
     }
...
function showStatePage(stateName) {
     ...
     const stateInitiatives = initiatives.filter(
          (initiative) =>
...
function handleRoute() {
     ...
     if (path === "/location" || path === "/location/") {
          // Check if the states list is already populated in HTML
          const existingStatesList = document.getElementById(
               "states-list-container"
          );
          if (
