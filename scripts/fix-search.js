...
const correctHeadScripts = `    <script src="/js/theme-init.js"></script>
    <!-- Critical Scripts -->
    <script type="module">
...
async function fixFile(filePath) {
     try {
          ...
          content = content.replace(
               /\s*<script type="module">\s*import { initializeSearch } from '\/components\/search-header\.js';\s*initializeSearch\(\);\s*<\/script>/g,
               ""
          );

          // Remove all existing component initialization scripts
          content = content.replace(
               /\s*<!-- Initialize components -->[\s\S]*?<script type="module">[\s\S]*?initializeComponents\({[\s\S]*?\}\);\s*<\/script>/g,
               ""
          );
          ...
          const initScript = correctInitScript.replace(
               "SIDEBAR_TYPE",
               sidebarType
          );
          ...
          content = content.replace(scriptRegex, correctScriptTag);

          // Ensure proper script loading order
          content = content.replace(
               /<script src="\/js\/script\.js"([^>]*)>/g,
               '<script src="/js/script.js" defer$1>'
          );
